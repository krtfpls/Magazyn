...
public class ProductValidator : AbstractValidator<ProductDto>
{
    ...
    {
        RuleFor(x => x.Name).NotEmpty().WithMessage("Uzupełnij nazwę!").MaximumLength(100).WithMessage("Max 100 letters!");
        ...
        RuleFor(x => x.Quantity).NotNull().GreaterThanOrEqualTo(0).LessThan(99999).WithMessage("Wrong quantity format!")
                .LessThanOrEqualTo(1).When(x => x.SerialNumber!.Length > 0);
        RuleFor(x => x.Description).MaximumLength(500).WithMessage("Max 500 letters!");
        RuleFor(x => x.CategoryName).NotEmpty().WithMessage("Uzupełnij kategorię!").MaximumLength(100).WithMessage("Max 100 letters!");
    }
}
